...
public class MushroomsPopulator extends BlockPopulator {
	...
	private int getHighestEmptyBlockYAtIgnoreTreesAndFoliage(World w, int x, int z){
		for (int y = w.getMaxHeight(); y >= 1; y--) {
			...
			Material material = handle.getType();
			if (material != Material.AIR &&  material != Material.LOG && material != Material.LEAVES && material != Material.YELLOW_FLOWER && material != Material.RED_ROSE) {
				return y;
			}
		}
		return 0;
